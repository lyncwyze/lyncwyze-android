...
class Activity_Activities_ChildList : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_activities_child_list)
        setupUI()
        setUpFunctionality()
    ...
    private fun fetchAllChildren() {
        loader.showLoader(this@Activity_Activities_ChildList)
        lifecycleScope.launch {
            try {
                val response = NetworkManager.apiService.getChildren(pageSize = 1000, offSet = 0, sortOrder = "ASC")
                if (response.isSuccessful) {
                    loader.hideLoader(this@Activity_Activities_ChildList)
                    val childList = response.body()
                    if (childList != null)
                        displayChildrenList(childList.data)
                    else
                        showToast(this@Activity_Activities_ChildList, "Fetching error!")
                } else {
                    loader.hideLoader(this@Activity_Activities_ChildList)
                    showToast(this@Activity_Activities_ChildList, "${getString(R.string.error_txt)}: ${response.code()}, ${HttpUtilities.parseError(response)?.errorInformation?.errorDescription}")
                }
            } catch (e: Exception) {
                logger.error { e.message }
                loader.hideLoader(this@Activity_Activities_ChildList)
                showToast(this@Activity_Activities_ChildList, "Exception: ${e.message}")
            }
        }
    }
    private fun displayChildrenList(children: List<SaveChild>) {
        ...
        if (children.isNotEmpty()) {
            for (child in children) {
                ...
                "${child.firstName} ${child.lastName}".also { textViewName.text = it }
                    ...
                    lifecycleScope.launch {
                        ImageUtilities.imageFullPath(endpoint)?.let { base64Data ->
                            try {
                                ...
                                Glide.with(this@Activity_Activities_ChildList)
                                    .load(bitmap)
                                    .into(imageViewChild)
                            } catch (e: Exception) {
                                e.printStackTrace()
                ...
                childItemView.setOnClickListener {
                    val intent = Intent(this, Activity_Activities_List::class.java)
                        .apply {
