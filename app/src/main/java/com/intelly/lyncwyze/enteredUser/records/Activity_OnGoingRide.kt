...
class Activity_OnGoingRide : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_on_going_ride)
        setupUI()
        setUpFunctionality()
    ...
    private fun fetchOngoingRides() {
        lifecycleScope.launch {
            try {
                loader.showLoader(this@Activity_OnGoingRide)
                val response = NetworkManager.apiService.getUpcomingRides(pageSize = 100, status = RideStatus.ONGOING.name, role= null)
                if (response.isSuccessful) {
                    loader.hideLoader(this@Activity_OnGoingRide)
                    response.body()?.let {
                        eachRide = it
                        displayRides()
                    } ?: showToast(this@Activity_OnGoingRide, "No upcoming rides found!")
                } else {
                    loader.hideLoader(this@Activity_OnGoingRide)
                    showToast(this@Activity_OnGoingRide, "${getString(R.string.error_txt)}: ${response.code()}, ${HttpUtilities.parseError(response)?.errorInformation?.errorDescription}")
                }
            } catch (e: Exception) {
                loader.hideLoader(this@Activity_OnGoingRide)
                logger.error { e.message }
                showToast(this@Activity_OnGoingRide, "Server error: ${e.message}")
                showNothingFound()
            }
        }
    }
        ...
        if (eachRide.data.isNotEmpty()) {
            ...
            for (rideD in eachRide.data) {
                ...
                rideGiverItemView.findViewById<ImageView>(R.id.callPerson).setOnClickListener {
                    ...
                    number?.let { startActivity(Intent(Intent.ACTION_DIAL).apply { data = Uri.parse("tel:$it") }) } ?: showToast(this, "Phone number is missing!")
                }
                    ...
                    lifecycleScope.launch {
                        ImageUtilities.imageFullPath(endpoint)?.let { base64Data ->
                            try {
                                ...
                                Glide.with(this@Activity_OnGoingRide).load(bitmap).into(personImg)
                            }
                            catch (e: Exception) {
                                e.printStackTrace()
