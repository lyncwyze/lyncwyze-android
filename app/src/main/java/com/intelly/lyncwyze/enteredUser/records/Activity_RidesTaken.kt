...
class Activity_RidesTaken : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_rides_taken)
        setupUI()
        setUpFunctionality()
    ...
    private fun fetchRides() {
        lifecycleScope.launch {
            try {
                loader.showLoader(this@Activity_RidesTaken)
                val response = NetworkManager.apiService.getUpcomingRides(pageSize = 100, status = RideStatus.COMPLETED.name, role = RiderType.TAKER.name)
                if (response.isSuccessful) {
                    loader.hideLoader(this@Activity_RidesTaken)
                    response.body()?.let {
                        eachRide = it
                        displayRides()
                    } ?: showToast(this@Activity_RidesTaken, "No upcoming rides found!")
                } else {
                    loader.hideLoader(this@Activity_RidesTaken)
                    showToast(this@Activity_RidesTaken, "${getString(R.string.error_txt)}: ${response.code()}, ${HttpUtilities.parseError(response)?.errorInformation?.errorDescription}")
                }
            } catch (e: Exception) {
                loader.hideLoader(this@Activity_RidesTaken)
                logger.error { e.message }
                showToast(this@Activity_RidesTaken, "Exception: ${e.message}")
            }
        }
    }
    private fun displayRides() {
        ...
        if (eachRide.data.isNotEmpty()) {
            ...
            for (rideD in eachRide.data) {
                ...
                rideGiverItemView.findViewById<TextView>(R.id.otherPartyName).text = "${rideD.userFirstName} ${rideD.userLastName}"
                ...
                rideGiverItemView.findViewById<TextView>(R.id.communityText).text  = "Community name"
                    ...
                    lifecycleScope.launch {
                        ImageUtilities.imageFullPath(endpoint)?.let { base64Data ->
                            try {
                                ...
                                Glide.with(this@Activity_RidesTaken)
                                    .load(bitmap)
                                    .into(personImg)
                            } catch (e: Exception) {
                                e.printStackTrace()
                ...
                rideGiverItemView.setOnClickListener {
                    val data = FeedBackPreReq(
                        rideId = rideD.id,
                        forUserId = rideD.userId,
                        forUserName = rideD.userFirstName + " " + rideD.userLastName,
                        fromUserId = rideD.rideTakers[0].userId,
                        fromUserName = rideD.rideTakers[0].userFirstName + " " + rideD.rideTakers[0].userLastName,
                        riderType =  RiderType.TAKER
                    )
            ...
            showToast(this@Activity_RidesTaken, getString(R.string.found_nothing))
        }
    }
}